...
To run the agent simply follow the instructions in each cell to download dependencies, and see the state statistics and take a random action in the environment. To train the agent run the cell with "maddpg" method which is a runner function for the multi agent. 
...
To train the agent allover, please refer to `Tennis (Run Code).ipynb`

The solution is described in detail in file `Project report.PDF`.

To load saved weights and watch the trained agent perform, please refer to the files `checkpoint_actor_1.pth`, `checkpoint_critic_1.pth`, `checkpoint_actor_2.pth` and `checkpoint_critic_2.pth`.
